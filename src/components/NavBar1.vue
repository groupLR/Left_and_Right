<script setup>
import SmallCart4 from './SmallCart4.vue';
...
<template>
  ...
  <div class="navbar">
    ...
    <div class="navbarMenu">
      ...
      <ul class="gap-5 navbarServices">
        ...
        <li><label for="cartSidebarSwitch" @click="toggleCart" style="cursor: pointer;"><font-awesome-icon :icon="['fas', 'bag-shopping']" /><span v-if="cartItemCount > 0" class="cart-count">{{ cartItemCount }}</span></label></li>

        <!-- 引入小購物車並綁定購物車顯示狀態 -->
    <SmallCart4 :isCartOpen="isCartOpen" @close="closeCart" />
        <li>
  ...
  <article class="sidebar">
    ...
    <li class="relative listItems">
      ...
      <label for="chevronSwitch">
        <font-awesome-icon
          ...
          class="absolute chevronIcon right-1 top-5"
        />
      </label>
      <ul class="chevron">
    ...
    <li class="relative listItems chevronIcon">
      <label for="sidebarLanguageSwitch">
      <span>繁體中文</span>
      <font-awesome-icon
        class="absolute globe-icon right-1 top-5 "
        :icon="['fas', 'globe']"/>
      </label>
      <input
        type="checkbox"
        name="sidebarLanguageSwitch"
        id="sidebarLanguageSwitch"
        class="sidebarLanguageSwitch"
      />
      <article class="px-5 pt-4 pb-3 sidebarLanguage">
        <ul>
...
.globe-icon {
  font-size: var(--font-size); /* 調整地球圖示大小 */
  color: #333;
  cursor: pointer; /* 手形指標樣式 */
}
...
.sidebarLanguageSwitch:checked + .sidebarLanguage {
  transform: translateX(0);
  transition: all 0.5s;
  height: 100vh;
}
