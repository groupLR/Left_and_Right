...
const toggleCart = () => {
  // 開關小購物車
  productStore.isCartOpen = !productStore.isCartOpen;
};
// 控制小購物車顯示/隱藏
const isCartOpen = ref(false);
const cartItems = ref([ /* 你的購物車項目 */ ]);
// 控制購物車的顯示狀態
...
const handleClickOutside = (event) => {
  const cartDropdown = document.querySelector('.cartPanel');
  const cartButton = document.querySelector('.cart-icon');
  if (isCartVisible.value && !cartDropdown.contains(event.target) && !cartButton.contains(event.target)) {
    closeCart(); // 關閉購物車;
  }
};
