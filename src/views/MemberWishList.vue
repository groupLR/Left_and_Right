<template>
    ...
    <div class="wishlistMain mx-auto p-10">
        <div
            class="wishlistTitle grid grid-cols-12 items-center border-b pb-2 text-[12px] font-semibold"
        >
        ...
        >
            <div class="commodityImg col-span-2 px-[15px]">
                <img
                    ...
                    "
                    :alt="
                        item.products.product_images[0].alt_text || '商品圖片'
                    "
                />
            </div>
...
const UID = "UID"; // 給localstroage一個變數
const wishlists = ref([]);

function resolveImagePath(path) {
    return `http://localhost:3300${path.replace("./", "/")}`;
}

onMounted(async () => {
    //抓UUID
    const memberUUID = localStorage.getItem(UID);

    try {
        const response = await axios.get(
            `http://localhost:3300/wishlist/${memberUUID}`
        );
...
const removeItem = async (id) => {
    ...
    try {
        await axios.delete(`http://localhost:3300/wishlist/delete/${id}`);
